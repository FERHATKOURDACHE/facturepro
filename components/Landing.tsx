    ...
    <main>
      <header className="mx-auto flex max-w-7xl items-center justify-between px-6 py-6">
        <Link href="/" className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">Facturation SaaS</p>
          </div>
        </Link>

        <Link
          href="/dashboard"
          className="rounded-full bg-slate-950 px-6 py-3 font-bold text-white"
        >
      ...
      <section className="mx-auto grid max-w-7xl items-center gap-12 px-6 pb-20 pt-10 lg:grid-cols-[1.1fr_0.9fr]">
        <div>
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-white/70 px-4 py-2 text-sm font-bold text-emerald-800">
            <BadgeCheck size={18} />
            Plateforme de facturation pour indépendants
          </div>

          <h1 className="max-w-5xl text-5xl font-black leading-[1.02] tracking-tight md:text-7xl">
            Le logiciel qui automatise tes{" "}
            <span className="gradient-title">factures</span>.
          </h1>

          <p className="mt-7 max-w-2xl text-lg leading-8 text-slate-600">
            Centralise tes clients, missions, heures, frais, factures,
            exports PDF/Excel et suivi de paiement dans un espace professionnel.
          </p>
        ...
        <div className="card rounded-[2rem] p-6">
          <div className="rounded-[1.5rem] bg-slate-950 p-6 text-white">
            <p className="text-sm text-slate-400">Facture prête</p>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-20">
        <div className="grid gap-5 md:grid-cols-4">
          {[
            [FileText, "Factures", "Numérotation, lignes, statuts et exports"],
            [Timer, "Heures", "Suivi par jour, semaine, client et lieu"],
            [WalletCards, "Frais", "Essence, repas, déplacement et notes"],
            [ShieldCheck, "Pilotage", "Profil entreprise, URSSAF et paiements"],
          ].map(([Icon, title, text]) => {
